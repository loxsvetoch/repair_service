...
from serviceproj.models import Employee, Service, WorkshopService, ServiceList, Role, Device, ServiceDevice, Order, OrderServices, Savepoint
    ...
    if request.method == 'POST':
        ...
        if form_type == 'employee_form':
            ...
            employee = Employee.query.filter_by(
                phone_number=employee_number).first()
            if todo == manage_type[0]:
                ...
                if not employee:
                    ...
                    db.session.commit()
                    flash(f"Новый работник {employee_first_name} {
                          employee_last_name} добавлен")
                else:
                ...
                Employee.query.filter(
                    Employee.phone_number == employee_number).delete()
        ...
        # Услуги
            ...
            if todo == manage_type[0]:
                service_idx = request.form.get(
                    "service_index_for_details")  # !!!!!
                workshop_service_desc = request.form.get('service_description')
                price = int(request.form.get('service_price'))
                service_id = Service.query.filter_by(
                    box_index=service_idx).first().id
            ...
            service_index_for_device = request.form.get(
                "service_index_for_device")
            ...
            db.session.commit()
            service = service_id = Service.query.filter_by(
                box_index=service_index_for_device).first()
