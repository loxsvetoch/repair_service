...
from serviceproj.models import Device, ServiceDevice, Service, WorkshopService, Role, Order
...
def get_filtered_data(service_id):
    devices = db.session.query(Device).join(ServiceDevice).filter(
        ServiceDevice.service_id == service_id).all()
    workshop_service_data = db.session.execute(text(
        "SELECT * FROM get_workshop_services(:service_id)"), {'service_id': service_id}).fetchall()
    workshop_services = [
...
def make_order():
    ...
    if request.method == 'POST':
        ...
        service = Service.query.filter(
            Service.box_index == service_idx).first()
...
def filter_data():
    ...
    selected_service = Service.query.filter_by(
        box_index=service_address.split(' ')[-1]).first()
